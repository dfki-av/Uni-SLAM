...
def get_sample_uv(H0, H1, W0, W1, n, b, depths, colors, device='cuda:0'):
    ...
    """
    if H0==0 and W0==0:                      # 这就是mapping的情况， 不需要剪切
        pass
    else:
...
def get_samples_all(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2ws, depths, colors, device, rays_d):

    b = c2ws.shape[0]
    indices = torch.randint(depths.shape[1], (n * b,), device=device)   
    indices = indices.reshape(b, -1)
    sample_depth = torch.gather(depths, 1, indices)  # (b, n)
    sample_color = torch.gather(colors, 1, indices.unsqueeze(-1).expand(-1, -1, 3))  # (b, n, 3)

    dirs = rays_d.unsqueeze(-2)
    rays_d = torch.sum(dirs * c2ws[:, None, :3, :3], -1)
    rays_o = c2ws[:, None, :3, -1].expand(rays_d.shape)
    rays_d = torch.gather(rays_d, 1, indices.unsqueeze(-1).expand(-1, -1, 3))
    rays_o = torch.gather(rays_o, 1, indices.unsqueeze(-1).expand(-1, -1, 3))
